...
if __name__ == '__main__':
    ...
    case_dir = '/cluster/scratch/lishi/dfn3_size1_xyz_dr2_2'
    max_iter = 500
    dodr = 2
    mv = 'D'
    ...
    stations = [(0.4, 0, 0.2), (-0.1, -0.2, 0.2), (0.4, -0.4, 0.2),
                (0.1, 0, -0.2), (0.3, 0.05, -0.2), (-0.15, -0.08, 0),(0.2, 0, 0)]

    # Setup dfn : | x | y | z | phi | psi | radius | aspect_ratio | beta | n_vertices |
    dfn_synthetic = np.asarray(
        [[-0.2, 0, 0, 0, np.pi / 2, 0.6, 0.8, 0, 5],
         [0.3, -0.3, 0.2, 0, np.pi / 2, 0.8, 0.7, 0, 5],
         [0.4, 0.2, -0.2, 0, np.pi / 2, 0.8, 0.8, 0, 5],
         [0.1, 0, 0.2, 0, np.pi/18, 0.8, 0.5, 20, 5]])

    dfn_initial = np.asarray(
        [[-0.2, 0, 0, 0, np.pi / 2, 0.6, 0.8, 0, 5],
         [0.3, -0.3, 0.2, 0, np.pi / 2, 0.8, 0.7, 0, 5],
         [0.4, 0.2, -0.2, 0, np.pi / 2, 0.8, 0.8, 0, 5],
         [-0.2, -0.4, -0.1, 0, np.pi/18, 0.8, 0.5, 20, 5]])
