...
if __name__ == '__main__':
    ...
    rms_0, rms_1, ratio_0 = likelihood_ratio(syn_data_old, syn_data_old, obs_data)
    status = {'model_id': 0, 'RMS': rms_0, 'fractures': s_current}
    dfninv.save_accepted_model(status, model_id=0)
    ...
    while i_iter < max_iteration:
        ...
        rms_old, rms_new, ratio_ll = likelihood_ratio(syn_data_old, syn_data_new, obs_data)
        ratio_prior = prior_new / prior_old
        ratio_proposal = 1

        #Matrix_normal_distribution(s_current, sigma_f, sigma_v).get_density(s_new)

        # calculate acceptance ratio
        Jacob = 1
        alpha_1 = min(1, ratio_ll * ratio_prior * ratio_proposal * Jacob)
        ...
        if alpha_1 > random():
            ...
            s_current = s_new
            prior_old = prior_new
            print('### Accept! RMS = {} ###'.format(rms_new))
            status = {'model_id': accept_case, 'RMS': rms_new, 'fractures': s_current}
            dfninv.save_accepted_model(status, model_id=accept_case)
